...
load ~/mdata/A_11feb2012  %% choose between the 4 deg and 2 deg MAT-files
load ~/Dropbox/ref_solution_11feb2012.mat A_ref

A = A_ref;
A_orig = A;
% lets pick a point.
pt = find(LAT(jt) == -60 & LON(it)==330 & DEPTH(kt)==2500)
...
for zz = 1:29
  ...
  A = A_orig;
  nz = find(LAT(jt) == -76 & LON(it)==330 & kt == zz);
  A(nz,:) = 0;
  A(nz,nz) = 1;
  [L, U, P, Q, R] = lu (A);
  vtot = R' \ (P' * (L' \ (U' \ (Q' * v))));  
...
Vtot = vector_to_field(vtot,it,jt,kt);
...
%% follow the idea in adjoint advection diffusion equation
%% calculations.
...
[L2, U2, P2, Q2, R2] = lu (A2) ;
%v2 = v;
%v2(kt==1) = vtot(kt==1);

vtot2 = R2' \ (P2' * (L2' \ (U2' \ (Q2' * v))));
Vtot2 = vector_to_field(vtot2,i,j,k);
contourf(LON,LAT,Vtot2)
